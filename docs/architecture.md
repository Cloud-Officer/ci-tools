...
- `build.yml`: Fan-out pipeline whose `variables` job (`cloud-officer/ci-actions/variables`) computes the `SKIP_LINTERS` / `SKIP_LICENSES` / `SKIP_TESTS` / `LINTERS` flags consumed by every downstream job; runs `actionlint`, `hadolint`, `markdownlint`, `rubocop`, `semgrep`, `trivy`, `yamllint`, a licenses/SOUP check, and the `shell_script_unit_tests` (Bats) and `ruby_unit_tests` (RSpec) jobs across a `macos-26` and `ubuntu-latest` matrix
