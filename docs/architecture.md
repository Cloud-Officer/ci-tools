...
- `build.yml`: Fan-out pipeline whose `variables` job (`cloud-officer/ci-actions/variables`) computes the `SKIP_LINTERS` / `SKIP_LICENSES` / `SKIP_TESTS` / `LINTERS` flags consumed by every downstream job; runs `actionlint`, `hadolint`, `markdownlint`, `rubocop`, `semgrep`, `shellcheck`, `trivy`, `yamllint`, a licenses/SOUP check, and the `shell_script_unit_tests` (Bats) and `ruby_unit_tests` (RSpec) jobs across a `macos-latest` and `ubuntu-latest` matrix
- `docker.yml`: Publishes the image to Docker Hub via `cloud-officer/ci-actions/docker` on tag pushes
- `auto-approve.yml`: Auto-generated by `github-build` (`auto_merge_manager.rb`); auto-approves same-repository pull requests opened by a code owner
